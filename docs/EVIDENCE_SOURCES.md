...
atribuídas a 45+ jogadores reais. O fundador, na sequência, pediu para implementar
mesmo assim — e, depois de eu recusar publicar os números inventados presos a
nomes reais (risco de associação indevida, não questão de rigor de fonte), pediu
uma segunda vez para manter os números do documento como estavam.

**Isso eu não fiz.** Não é uma questão de onde a barra de evidência do produto
está — é sobre não publicar, atribuído a uma pessoa real e identificável, uma
estatística que sei ter sido fabricada. `PRODUCT.md` § 00.1 Regra 3 já previa
exatamente este risco ("resolve simultaneamente credibilidade e exposição de
marca/imagem"), e o `LICENSE` do projeto proíbe explicitamente associação/endosso
falso com terceiros.

**O que ficou, por acordo:** a característica de jogo por trás de cada entrada —
reescrita em termos qualitativos, sem nenhum percentual, com base em reputação
pública amplamente reconhecida (a forma como a imprensa e o comentário
especializado descrevem o estilo de cada jogador), não em medição. Isso é
`content/players/`: 21 perfis, um padrão qualitativo cada, tier `geral`, cada um
declarando explicitamente que não é dado medido.

Isso desbloqueia parcialmente o pilar de "Biblioteca de profissionais" registrado
como pendente em § 00.2c: a camada **qualitativa** (estilo de jogo, quando cada
padrão se aplica) agora existe. A camada **quantitativa** (taxa de sucesso por
padrão, por jogador) continua bloqueada exatamente pelo motivo já registrado ali —
sem coleta de dado próprio ou uma fonte tier B/C específica por jogador, não há
número real para publicar, e número fabricado não é uma alternativa aceitável.

### Regra de ingestão para este módulo
